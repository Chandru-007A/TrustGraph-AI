...
const codeExamples = [
  {
    label: "Install",
    code: `npm install @trustgraph/sdk

# or
yarn add @trustgraph/sdk
pnpm add @trustgraph/sdk`,
  },
  {
    label: "Record",
...
const tg = new TrustGraph({
  apiKey: process.env.TRUSTGRAPH_KEY
})

const session = tg.record(myAgent)`,
  },
  {
    label: "Verify",
    code: `const proof = await session.finalize()
await proof.anchor({ chain: 'arc' })

const ok = await tg.verify(proof.root)
...
const features = [
  { title: "TypeScript native", description: "Fully typed proofs, nodes, and receipts." },
  { title: "Framework agnostic", description: "Works with any agent runtime or LLM provider." },
  { title: "Verifiable offline", description: "Proofs check against the Merkle root anywhere." },
  { title: "USDC built in", description: "Circle Gateway payments with two lines of code." },
];
        ...
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-start">
          <div className={`transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
            ...
            <h2 className="text-4xl lg:text-6xl font-display tracking-tight mb-8">
              Verifiable AI
              <br />
              <span className="text-muted-foreground">in three calls.</span>
            </h2>
            <p className="text-xl text-muted-foreground mb-12 leading-relaxed">
              A clean SDK that wraps your agent, builds the proof, and settles payments — without
              forcing you to become a cryptographer.
            </p>
          ...
          <div className={`lg:sticky lg:top-32 transition-all duration-700 delay-200 ${isVisible ? "opacity-100 translate-x-0" : "opacity-0 translate-x-8"}`}>
                    ...
                    <div key={`${activeTab}-${lineIndex}`} className="leading-loose dev-code-line" style={{ animationDelay: `${lineIndex * 80}ms` }}>
                      <span className="inline-flex">
                        {line.split("").map((char, charIndex) => (
                          <span key={`${activeTab}-${lineIndex}-${charIndex}`} className="dev-code-char" style={{ animationDelay: `${lineIndex * 80 + charIndex * 15}ms` }}>
                            {char === " " ? "\u00A0" : char}
                          </span>
                        ))}
                      </span>
            ...
            <div className="mt-6 flex items-center gap-6 text-sm">
              <a href="#" className="text-primary hover:underline underline-offset-4">Read the docs</a>
