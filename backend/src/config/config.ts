...
const envVarsSchema = z.object({
  ...
  ARC_RPC_URL: z.string().url().optional().or(z.literal('')).describe('Arc L1 JSON-RPC endpoint'),
  ARC_PRIVATE_KEY: z.string().optional().describe('Operator private key for signing transactions'),
  RECEIPT_REGISTRY_ADDRESS: z.string().optional().describe('Deployed ReceiptRegistryV2 contract address'),
  ARC_EXPLORER_URL: z.string().optional().describe('ArcScan base URL for explorer links'),

  // x402 Payment Protocol Configuration
  X402_FACILITATOR_URL: z.string().url().optional().or(z.literal('')).describe('Coinbase x402 facilitator API endpoint'),
  X402_NETWORK: z.string().default('eip155:8453').describe('CAIP-2 blockchain identifier'),
  X402_SELLER_ADDRESS: z.string().default('0x5e11e80000000000000000000000000000000000').describe('Seller/merchant payout address'),
  USDC_TOKEN_ADDRESS: z.string().default('0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913').describe('USDC ERC-20 contract address'),
  ...
  // real keys.
  GATEWAY_ENABLED: z.string().optional().describe('Set to "true" to enable live Circle Gateway (default: mock mode)'),
  KIT_KEY: z.string().optional().describe('Circle App Kit API key (x-app-kit-key) — enables live App Kit'),
  CIRCLE_GATEWAY_KEY: z.string().optional().describe('Circle Gateway API key — used for webhook signature validation'),
  GATEWAY_RPC_URL: z.string().url().optional().or(z.literal('')).describe('Custom Arc Testnet RPC override (defaults to public Arc RPC)'),
  GATEWAY_WALLET_PRIVATE_KEY: z.string().optional().describe('Operator EVM private key for signing Gateway operations. Never commit.'),
  GATEWAY_WEBHOOK_SECRET: z.string().optional().describe('HMAC-SHA256 secret for verifying Circle Gateway webhook payloads'),
  GATEWAY_DELEGATE_ADDRESS: z.string().optional().describe('Address delegated to spend from operator balance (defaults to operator EOA)'),
  UNIFIED_BALANCE_NETWORK: z.string().default('Arc_Testnet').describe('Circle Unified Balance network identifier (Arc_Testnet | Arc)'),
});
